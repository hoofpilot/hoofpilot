...
from hoofpilot.navd.helpers import Coordinate, bearing_between_two_points, distance_along_geometry, string_to_direction
...
class NavigationInstructions:
  ...
  def get_route_progress(self, current_lat, current_lon) -> dict | None:
    ...
    self.closest_idx, min_distance = min(
      ((idx, self.coord.distance_to(coord)) for idx, coord in enumerate(route['geometry'])),
      key=lambda x: x[1],
    )
    closest_cumulative = distance_along_geometry(route['geometry'], self.coord)

    # Find the current step index: highest idx where step cumulative distance <= closest cumulative
    current_step_idx = max(
      (idx for idx, step in enumerate(route['steps']) if step['cumulative_distance'] <= closest_cumulative),
      default=-1,
    )
  ...
  def get_current_route(self):
    ...
    cumulative_distances.extend(
      cumulative_distances[-1] + geometry[step - 1].distance_to(geometry[step])
