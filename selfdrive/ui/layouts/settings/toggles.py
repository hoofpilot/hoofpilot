...
DESCRIPTIONS = {
  "OpenpilotEnabledToggle": tr_noop(
    "Use the hoofpilot system for adaptive cruise control and lane keep driver assistance. " +
    "Your attention is required at all times to use this feature."
  ),
  "DisengageOnAccelerator": tr_noop("When enabled, pressing the accelerator pedal will disengage hoofpilot."),
  "LongitudinalPersonality": tr_noop(
    "Standard is recommended. In aggressive mode, hoofpilot will follow lead cars closer and be more aggressive with the gas and brake. " +
    "In relaxed mode hoofpilot will stay further away from lead cars. On supported cars, you can cycle through these personalities with " +
  ...
  "AlwaysOnDM": tr_noop("Enable driver monitoring even when hoofpilot is not engaged."),
...
class TogglesLayout(Widget):
  def __init__(self):
    ...
    for param, (title, desc, icon, needs_restart) in self._toggle_defs.items():
      ...
      if needs_restart and not locked:
        additional_desc = tr("Changing this setting will restart hoofpilot if the car is powered on.")
  ...
  def _update_toggles(self):
    ...
    e2e_description = tr(
      "hoofpilot defaults to driving in chill mode. Experimental mode enables alpha-level features that aren't ready for chill mode. " +
      "Experimental features are listed below:<br>" +
      "<h4>End-to-End Longitudinal Control</h4><br>" +
      "Let the driving model control the gas and brakes. hoofpilot will drive as it thinks a human would, including stopping for red lights and stop signs. " +
    ...
    if ui_state.CP is not None:
      if ui_state.has_longitudinal_control:
        ...
        long_desc = unavailable + " " + tr("hoofpilot longitudinal control may come in a future update.")
        if ui_state.CP.alphaLongitudinalAvailable:
          if self._is_release:
            long_desc = unavailable + " " + tr("An alpha version of hoofpilot longitudinal control can be tested, along with " +
                                               "Experimental mode, on non-release branches.")
          else:
            long_desc = tr("Enable the hoofpilot longitudinal control (alpha) toggle to allow Experimental mode.")
